...
const useDeviceSelect = (deviceType: deviceType) => {
  ...
  const deviceLabel = devices?.[idx.row]?.label ?? (
    // Graceful label fallback for most browsers
    // (i.e. no support for audio output device selection)
    isAudioOutput
  ...
  useEffect(() => {
    ...
    const isDifferentDevice = (localTrackLabel && nextDeviceLabel) && (localTrackLabel !== nextDeviceLabel)
    const isResumingAbortedDevice = (deviceAborted && nextDeviceLabel) && (deviceAborted === nextDeviceLabel)
  ...
  return {
    ...
    setIdx: (index: IndexPath | IndexPath[]) => {
      ...
      if (!Array.isArray(index)) setIdx(index)
    },
    devices,
    deviceLabel,
  }
